...
final class Version20260527000001 extends AbstractMigration
{
    ...
    {
        return 'Convert security_logs.action from VARCHAR to SMALLINT UNSIGNED using SecurityLogAction enum values';
    }

    public function up(Schema $schema): void
    {
        $this->addSql("ALTER TABLE security_logs ADD action_code SMALLINT UNSIGNED DEFAULT NULL");

        $this->addSql("UPDATE security_logs SET action_code = CASE action
        ...
        $this->addSql('ALTER TABLE security_logs CHANGE action_code action SMALLINT UNSIGNED NOT NULL');
        $this->addSql('CREATE INDEX IDX_security_log_action ON security_logs (action)');
    }

    public function down(Schema $schema): void
    {
        $this->addSql("ALTER TABLE security_logs ADD action_text VARCHAR(100) DEFAULT NULL");

        $this->addSql("UPDATE security_logs SET action_text = CASE action
        ...
        $this->addSql('DROP INDEX IDX_security_log_action ON security_logs');
        $this->addSql('ALTER TABLE security_logs DROP COLUMN action');
        $this->addSql('ALTER TABLE security_logs CHANGE action_text action VARCHAR(100) NOT NULL');
        $this->addSql('CREATE INDEX IDX_security_log_action ON security_logs (action)');
    }
}
